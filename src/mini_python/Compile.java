...
class Compile {
	...
	static X86_64 file(TFile f) {
		...
		// Because the main function is not called via TEcall, its local stack variables
		// will not be allocated by our Compiler class
	 ...
	 * <p>
	 * NOTE: because all arguments are evaluated and pushed to the stack, and then
	 * copied to local variables also
	 * allocated on the stack, we do not need to try and optimize the function call
	 * using registers.
	 * <p>
	 * As an optimization, we make the local variables of all functions (except
	 * main) refer to the stack positions before
	 * the new stack frame and return value (ie 16(%rbp), 24(%rbp), etc instead of
	 * -8(%rbp), -16(%rbp), etc) because
	 * we are essentially duplicating the stack frame for each function call.
	 */
	static public void acceptFunc(Compiler compiler, TDef def) {
		...
		// We do not preallocate stack space, as we will just push values from the
		// argument stack frame
		...
		// Move all arguments to the new stack frame. The 1st argument is the last that
		// was pushed to the stack
		// (so that depending on our optimization strategy, we have the choice to reuse
		// the previous stack frame,
		// with arguments having their absolute stack frame offset always increasing in
		// the order of arguments).
		// for (int i = 0; i < argCount; i++) {
		// WE DO THE OPTIMIZATION ! ˇˇˇˇˇˇˇˇˇˇˇˇ
		// NOTE: if we reused the previous stack frame, we would save <arg_count>
		// instructions.
		// compiler.x86_64.pushq((i + 2) * 8 + "(%rbp)"); // Push the argument to the
		// stack
		// }

		// Allocate stack space for the local variables of the function
		// Force allocation of an even number of local variables to ensure stack
		// alignment
		if (-def.f.localVariablesOffset % 16 != 0) {
			def.f.localVariablesOffset -= 8;
		}

		int additionalOffset = def.f.localVariablesOffset; // Do not add parameters, that were already added to the
															// stack frame
		if (additionalOffset != 0) {
			compiler.x86_64.subq("$" + -additionalOffset, Regs.RSP);
		}

		// Accept the function body. It contains return statements, we do not need to
		// restore the stack frame
		// and dealloc local variables here, the return statements will handle it in our
		// compiler.
		compiler.currentFunction = def; // Set the current function reference for cleaning up the correct stack size
										// upon returning.
